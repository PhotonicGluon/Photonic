---
title: "Security over Insecurity: The Foundations of Authenticated Key Exchange Protocols"
pubDate: 2025-07-04
summary: "A tour of the wonderful world of authenticated key exchange."
image:
...
As with all cryptography analogies, let us introduce Alice and Bob. Alice wants to send a message to Bob, but doesn't want anyone else from reading it. Of course, Alice and just encrypt the message before sending it to Bob, but Bob would need the key[^sym-enc] to retrieve the original key. So, how do they share the key?
...
The other critical observation underlying the foundation of Diffie-Hellman is the commutivity of exponentiation. In particular,
...
The textbook Diffie-Hellman Key Exchange (DHKE) relies on a publicly known prime $p$ and a generator $g$[^dhke-practical]. The key exchange protocol between two people, say Alice and Bob, proceeds as follows.
...
which is the same as $S_A$ (note that the modulo operations have been omitted for brevity). This means that, assuming that both sides have the correct values, the shared secrets should be the same. This shared value can then be used to derive a shared key (for example, by using the hash $H(\cdot)$ in step 3 on the shared value).
...
where $\oplus$ refers to the exclusive OR (XOR) operation. Alice sends her $M_1$ to Bob; Bob computes the received $M_1$ with his value. If they do not match, Bob knows that the other party does not know the 'password value' $x$ and halts. Otherwise, both parties will continue by calculating their own $M_2$ values
